...
// Задаем номер поpта и длину буфера
#define PORT    8767
#define BUFLEN  512
...
struct condition 
{
	...
	void make(const string &one_zapr)
	{
		...
		if(!comand_or_not(comand))
		{
			cout << "This is not a command - " << comand;
			throw MyException(-3, "This is not a command");
		}
 		i_copy = i;
		while(i < one_zapr.size() - 4)
		{
			...
			{
				throw MyException(-2, "Double space");
			}
			i++;
		}
		i = i_copy + 1;
		...
		if(comand == "save")
		{
			...
			{
				throw MyException(11, "Command \"SAVE\" specified incorrectly");
			}
		}
		else if(comand == "generate")
		{
			...
			{
				throw MyException(272, "Command \"GENERATE\" specified incorrectly");
			}
			while(i < one_zapr.size() - 4 )
			{
				if(one_zapr[i] == ' ')
				{
					throw MyException(312, "Command \"GENERATE\" specified incorrectly");
				}
				if(!number_or_not(one_zapr,i))
				{
					throw MyException(892, "Command \"GENERATE\" specified incorrectly");
				}
				cr.val += one_zapr[i];
				i++;
			}
		...
		else if(comand == "print")
		{
			while(i < one_zapr.size() - 3 )
			{
				if(one_zapr[i] == ' ')
				{
					if(!pole_or_not(cr.pole))
					{
						throw MyException(12, "The field is set incorrectly");
					}
					if(cr.pole[0] == 'd')
		 			{
		 				prov.date_time++;
			...
			if(prov.date_time > 1 || prov.group > 1 || prov.room > 1 || prov.subject > 1 || prov.teacher > 1)
			{
				...
				throw MyException(8, "Not all fields are specified");
			}
		}
		else if(comand == "select" || comand == "reselect" || comand == "insert" || comand == "remove")
		{
			...
			while(i < one_zapr.size() - 3)
			{
				...
				{
					cout << endl << cr.pole;
					throw MyException(-1, "The field is set incorrectly");
				}
				i++;
				i_copy = i;
				...
				else if(comand == "select" || comand == "reselect")
				{
					...
					while(one_zapr[i] != '-' && one_zapr[i] != ' ')
					{
						if(number_or_not(one_zapr,i) == 0)
						{
							...
							{
								throw MyException(4, "The number is incorrect");
							}
							poin = 1;
							if(one_zapr[i+1] != '-')
							{
								throw MyException(5, "The number is incorrect");
							}
						}
					...
					else if(one_zapr[i] == '-')
					{
						...
						while(one_zapr[i] != ' ')
						{
							if(number_or_not(one_zapr,i) == 0)
							{
								if(one_zapr[i] != '*' || poin == 1)
								{
									throw MyException(6, "The number is incorrect");
								}
								poin = 1;
								if(one_zapr[i+1] != ' ')
								{
									throw MyException(7, "The number is incorrect");
								}
							}
					...
					if(cr.pole == "date_time" &&  more(cr.high_val,"2400") == cr.high_val )
					{
						...
						throw MyException(23, "Time set incorrectcon");
					}
				...
				else if(comand == "insert" || comand == "remove")
				{
					while(one_zapr[i] != ' ')
					{
						if(number_or_not(one_zapr,i) == 0)
						{
							throw MyException(9, "The number is incorrect");
						}
						cr.low_val.push_back(one_zapr[i]);
						i++;
					}
...
struct resultat
{
    ...
    void print()
    {
    	if(stroka.empty())
	    {
	    	for(unsigned long int i = 0;i < inf.size();i++)
	    	{
	    			inf[i].print();
				}
			}
...
public:
	void make(const string &zapr) 
	{
		unsigned long int q = 0, i = 0;
		string one_zapr;
		condition s;
		if(zapr[zapr.size()-4] != ' ' || zapr[zapr.size()-3] != 'e' || zapr[zapr.size()-2] != 'n' || zapr[zapr.size()-1] != 'd')
		{
			throw MyException(13, "The request must have \"end\" at the end");
		}
		while(i < zapr.size()-3)   
		{
...
public:
  ...
  void push_ses()
  {
	...
	Database(const string &file_name)
	{
		...
		cout  << endl << "The base is over"  << endl;
	}
	...
	vector <int> association(condition c)
	{
		...
		for(unsigned long int i = 0;i < c.criteri.size();i++)
		{
	...
	resultat resresult_of_zapros(const condition &one_zapr,int number)
	{
		...
		else if(one_zapr.comand == "insert")
		{
		    ...
		    global_index++;
			add_elem(inf);
			otvet.stroka = "Element added";
		}
		else if(one_zapr.comand == "select")
		{
			unsigned long int i ,q = 0;
		...
		{
			unsigned long int i = 0, q = 0, keks = 0;
			proverka_insert_and_remove prov;
			prov.date_time = 0;
			prov.group = 0;
			prov.room = 0;
